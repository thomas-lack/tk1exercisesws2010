...
public class MandelMaster implements Callback
{
   ...
   **/
   ...
   public void connect(String host, int port) throws TupleSpaceException
   {
      ...
      template = new Tuple(new Field(masterID),new Field(MandelObject.class), new Field(boolean.class));
      
      /*
       * Filter template = (ID,index, x, y, result, processed) 
       */
   ...
   /**
    *  Function that sends a request to the t-space
    */
   
   public void send(int index, double x, double y)
   {
	  MandelObject MO = new MandelObject(index,x,y);
 	  try
 	  {
 		  tupleSpace.write(masterID, MO, false);
 	  }
 	  catch (TupleSpaceException e)
 	  {
